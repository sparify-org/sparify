...
import { ArrowLeft, Settings, ArrowUpRight, Target, Trophy, CheckCircle2, PiggyBank as PigIcon, Trash2, Signal, ArrowDownLeft, Wallet, Star, Flag, ArrowRightLeft, PieChart, Check, Sparkles, Gift, ShoppingBag, AlertCircle, PlusCircle, X, Percent, TrendingUp, Info, Lock } from 'lucide-react';
...
interface PiggyDetailScreenProps {
  ...
  onTransaction: (pigId: string, newBalance: number, transactions: Transaction[]) => void;
  onDeleteBank: (id: string) => void;
  onDeleteGoal: (pigId: string, goal: Goal) => void;
...
export const PiggyDetailScreen: React.FC<PiggyDetailScreenProps> = ({ bank, user, piggyBanks, onBack, onUpdateBank, onTransaction, onDeleteBank, onDeleteGoal, onUpdateGoal, onUpdateUser, language, appMode = 'kids' }) => {
  ...
  const handleTransactionExecute = () => {
    ...
    onTransaction(bank.id, currentBalance - amount, [newTransaction]);
               ...
               <div style={{ width: '100%', height: 140 }}>
                 <ResponsiveContainer width="100%" height="100%">
                   <AreaChart data={chartData}>
                     ...
                     <XAxis dataKey="day" axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 10, fontBold: 'bold' }} dy={10} />
            ...
            {showPayout && (
                <div className="fixed inset-0 z-[130] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-[2.5rem] w-full max-w-md p-8 animate-in slide-in-from-bottom-10 shadow-2xl">
                        <h3 className="text-2xl font-black text-slate-900 mb-2">{t.payoutTitle}</h3>
                        <p className="text-slate-500 font-bold mb-6">Kontostand: €{bank.balance.toFixed(2).replace('.', ',')}</p>
                        ...
                        <button onClick={handleTransactionExecute} className="w-full text-white font-black text-lg py-5 rounded-2xl shadow-xl active:scale-95 transition-all bg-amber-400 shadow-amber-200">{t.confirm}</button>
                        <button onClick={() => setShowPayout(false)} className="w-full mt-4 py-4 text-slate-400 font-bold">{t.cancel}</button>
                    </div>
                </div>
            )}
            {showGoalModal && (
